...
def convert_gdrive_url(url, for_download=False):
    ...
    if match:
        ...
        if for_download:
            ...
            return f"https://drive.google.com/embed/view?id={file_id}"
    return url
...
def index():
    ...
    featured_anime = [anime for anime in data['anime'] if anime.get('featured', False)]
...
def search():
    ...
    if query:
        anime_list = [anime for anime in anime_list if query in anime['title'].lower()]

    # Filter by genre if provided
    if genre:
        anime_list = [anime for anime in anime_list if genre in [g.lower() for g in anime.get('genres', [])]]
...
# Route pour télécharger le projet
@app.route('/download_project')
def download_project():
    # Chemin du fichier ZIP
    zip_path = 'anime-stream-essential.zip'

    # Si le fichier ZIP n'existe pas déjà, le créer
    if not os.path.exists(zip_path):
        try:
            # Préparation du dossier temporaire
            temp_dir = 'export_project'
            if not os.path.exists(temp_dir):
                os.makedirs(temp_dir)

            # Copier les fichiers essentiels dans le dossier temporaire
            essential_dirs = ['templates', 'static']
            essential_files = ['app.py', 'main.py', 'guide_ajout_anime.txt']

            # Copie des dossiers
            for directory in essential_dirs:
                if os.path.exists(directory):
                    subprocess.run(['cp', '-r', directory, temp_dir], check=True)
            ...
            with open(readme_path, 'w') as f:
                f.write("# Anime Stream\n\n")
                f.write("Un site de streaming d'anime développé avec Flask.\n\n")
                f.write("## Installation\n\n")
                f.write("1. Cloner le dépôt\n")
                f.write("2. Installer les dépendances : `pip install -r requirements.txt`\n")
                f.write("3. Exécuter l'application : `python main.py`\n\n")
                f.write("## Fonctionnalités\n\n")
                f.write("- Visualisation des animes\n")
                f.write("- Recherche par genre\n")
                f.write("- Lecture des épisodes\n")
            ...
            cwd = os.getcwd()
            os.chdir(temp_dir)
            subprocess.run(['zip', '-r', f'../{zip_path}', '.'], check=True)
            os.chdir(cwd)

        except Exception as e:
            logging.error(f"Erreur lors de la création du ZIP: {str(e)}")
            return "Une erreur s'est produite lors de la préparation du téléchargement.", 500

    # Envoyer le fichier ZIP existant
    return send_file(zip_path, as_attachment=True, download_name='anime-stream-project.zip')
