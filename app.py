...
def index():
    ...
    # Récupérer les animes en cours de visionnage (seulement les cartes anime)
    continue_watching = []
    if current_user.is_authenticated:
        # Récupérer les progressions les plus récentes par anime, en excluant les terminés
        latest_progress_by_anime = db.session.query(
            UserProgress.anime_id,
            db.func.max(UserProgress.last_watched).label('last_watched')
        ).filter_by(
            ...
            db.desc('last_watched')
        ).all()
        
        # Pour chaque anime, trouver les données et ajouter à la liste
        for anime_id, last_watched in latest_progress_by_anime:
            # Utiliser la même conversion d'ID que les autres fonctions
            anime = next((a for a in anime_data if int(a.get('id', 0)) == anime_id), None)
            if anime:
                # Récupérer la dernière progression pour cet anime en fonction de la date
                # et non du numéro d'épisode (pour afficher l'épisode regardé le plus récemment)
    ...
    try:
        ...
        for directory in essential_dirs:
            if os.path.exists(directory):
                shutil.copytree(directory, os.path.join(temp_dir, directory), dirs_exist_ok=True)
